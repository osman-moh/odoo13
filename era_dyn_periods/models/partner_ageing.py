...
class InsPartnerAgeing(models.TransientModel):
    ...
    def process_detailed_data(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        if partner:
            ...
            SELECT = """SELECT m.name AS move_name,
                                ...
                                m.payment_duration AS payment_duration,
                                m.inv_bill_duration AS inv_bill_duration,
    ...
    def get_report_datas(self, default_filters={}):
        r=self.env['account.move'].search([('state','=','posted')])
        for record in r:
            if record.invoice_date:
                today = fields.Date.today()#2022-12-01  - 2022-12-29        
                delta1 = today - record.invoice_date
                record.inv_bill_duration = delta1.days
                record.payment_duration=0    
            elif record.date and record.payment_state=='paid' :
                today = fields.Date.today()
                delta1 = today - record.date
                record.payment_duration = delta1.days 
                record.inv_bill_duration=0   
        '''
    ...
    def action_xlsx(self):
        ...
        if record.include_details:
            ...
            # sheet.write(row_pos, 2, _('Invoice/Bill Date'), format_header)
            # sheet.write(row_pos, 3, _('Inv/Bill Duration'), format_header)
            # sheet.write(row_pos, 4, _('Payment Date'), format_header)
            # sheet.write(row_pos, 5, _('Payment Duration'), format_header)
